...
「17 リフレクションとアノテーション」
●リフレクション
・リフレクションはクラスを作るための設計書の様なもの。
・クラスそのものの設計書となる特別クラスは、javaでは「java.lang.Class」クラスとして定義されている。
・プログラム実行時にはClassクラスのインスタンスとして扱うことができる。

【Classクラスのインスタンスを取得する】
①「.class」という構文を使用する方法。
→構文：Class<クラス名> 変数名 = クラス名.class
②インスタンスから取得する方法。
→構文：クラス名 インスタンスの変数名 = new クラス名();
      Class<? extends クラス名> 変数名 = インスタンスの変数名.getClass();
③クラスの名前から取得する方法。
→構文：Class<?> 変数名 = Class.forName("クラスの名前");

【Classクラスの情報】
・クラスの名前、親クラス、定義されているフィールド、定義されているコンストラクター、定義されているメソッドなどが記述されている。
